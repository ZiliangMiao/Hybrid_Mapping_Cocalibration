...
def nlmeans(edge_lid):
    edge_lid_u = edge_lid[:int(0.15 * edge_lid.shape[0]), :]
    edge_lid_l = edge_lid[int(0.15 * edge_lid.shape[0]):, :]
    edge_lid_u = cv2.fastNlMeansDenoising(edge_lid_u, h=40, searchWindowSize=21, templateWindowSize=7)
    edge_lid_l = cv2.fastNlMeansDenoising(edge_lid_l, h=20, searchWindowSize=21, templateWindowSize=7)
...
if __name__ == "__main__":
    ...
    for angle in gimbal_angles:
        ...
        dir_cam_mask = root_path + "/python_scripts/hog_filter/flatImage_mask.bmp"
        ...
        dir_cam_output = data_path + "/edges/canny_outputs/cam_edge.png"

        dir_lid_original = data_path + "/outputs/byIntensity/flatLidarImage.bmp"
        dir_lid_mask = root_path + "/python_scripts/hog_filter/flatLidarImage_mask" + str(angle) + ".png"
        ...
        dir_lid_output = data_path + "/edges/canny_outputs/lid_edge.png"
        ...
        if(angle == 0):
            ...
            edge_cam = cv2.GaussianBlur(edge_cam, sigmaX=5, sigmaY=1, ksize=(5, 5))
        ...
        cnt_lid = contour_filter(contour=cnt_lid, len_threshold=100)
